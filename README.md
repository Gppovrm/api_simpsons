...
из этой же библиотеки будем использовать Field, чтобы задать дополнительные ограничения на поля (например, максимальная
длина строки)
...
Для более структурированного кода и дальнейшей отладки будем использовать роутеры для каждой из моделей
...
`POST /api/characters` - добавление нового персонажа. В теле входящего запроса необходимо ожидать JSON структуры
объекта.
...
`PATCH /api/characters/{character_id}` - изменение информации о персонаже с указанным id. Входящий запрос и формат
ответов
